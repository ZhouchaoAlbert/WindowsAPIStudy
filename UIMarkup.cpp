...
bool CMarkup::_Parse(LPTSTR& pstrText, ULONG iParent)
{
    ...
    for( ; ; ) 
    {
        ...
        *pstrText++ = _T('\0'); //去掉 <  
        _SkipWhitespace(pstrText);  //去掉字符串中最前面含有空格以及"\0"的字符
        ...
        XMLELEMENT* pEl = _ReserveElement();  //返回当前控件所存的序号的位置  //节点存储，少了则扩展
        ULONG iPos = pEl - m_pElements;		  //获得控件序号
        pEl->iStart = pstrText - m_pstrXML;   //控件对应的字符串在整个xml中的偏移量，对应着控件字符串的开始
        pEl->iParent = iParent;				  //记录该节点的父节点序号
        pEl->iNext = pEl->iChild = 0;         //平级控件的下一个控件的序号，以及子控件队列中的首控件序号
        if( iPrevious != 0 )                  //如果iPrevious !=0 说明记录的是平级控件
			m_pElements[iPrevious].iNext = iPos; 
        else if( iParent > 0 )                //如果iPrevious ==0 说明记录的是平级控件队列的第一个，所以该节点的父节点记录子控件队列中的第一个控件序号。
			m_pElements[iParent].iChild = iPos;
        iPrevious = iPos;
        // Parse name
		LPCTSTR pstrName = pstrText;  //当前控件的类型名称的开头偏移量
        _SkipIdentifier(pstrText);	  //跳过控件类型
        LPTSTR pstrNameEnd = pstrText;//记录去控件类型后的偏移量
        if( *pstrText == _T('\0') ) 
			return _Failed(_T("Error parsing element name"), pstrText);
        // Parse attributes
        if( !_ParseAttributes(pstrText) ) return false;  //解析属性  //找到一个控件的字符串的最后一个字母的偏移位置
        _SkipWhitespace(pstrText);
        ...
        else  //这种情况下说明是有子控件队列的
        {
            ...
            if( !_ParseData(pstrText, pstrDest, _T('<')) ) return false;  //跳到最近的'<'的偏移位置
            // Determine type of next element
            if( *pstrText == _T('\0') && iParent <= 1 )  //结束
				return true;
            if( *pstrText != _T('<') ) 
