...
{
	ATLASSERT(!::IsBadStringPtr(lpsz, -1) || lpsz == NULL);//表示当前进程是否拥有字符串指针指向的字符串的度操作权限   
	m_szBuffer[0] = '\0';
	Assign(lpsz, nLen);
}
...
const CDuiString& CDuiString::operator=(LPCTSTR lpStr)
{
	if (lpStr)
	{
		ATLASSERT(!::IsBadStringPtr(lpStr, -1));
		Assign(lpStr);
	}
	else
	{
...
const CDuiString& CDuiString::operator=(LPCSTR lpStr)
{
	if (lpStr)
	{
		ATLASSERT(!::IsBadStringPtrA(lpStr, -1));
...
{
	ATLASSERT(nIndex >= 0 && nIndex < GetLength());
	m_pstr[nIndex] = ch;
}
...
{
	ATLASSERT(iPos >= 0 && iPos <= GetLength());
...
{
	ATLASSERT(!::IsBadStringPtr(pstrSub, -1));
	ATLASSERT(iPos >= 0 && iPos <= GetLength());
	if (iPos != 0 && (iPos < 0 || iPos > GetLength())) return -1;
